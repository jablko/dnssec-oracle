...
contract TestBytesUtils {
    ...
    function testBase32HexDecodeWord() public {
        Assert.equal(
            "C4".base32HexDecodeWord(0, 2),
            bytes32(byte("a")),
            "Decode 'a'"
        );
        Assert.equal(
            "C5GG".base32HexDecodeWord(0, 4),
            bytes32(bytes2("aa")),
            "Decode 'aa'"
        );
        Assert.equal(
            "C5GM2".base32HexDecodeWord(0, 5),
            bytes32(bytes3("aaa")),
            "Decode 'aaa'"
        );
        Assert.equal(
            "C5GM2O8".base32HexDecodeWord(0, 7),
            bytes32(bytes4("aaaa")),
            "Decode 'aaaa'"
        );
        Assert.equal(
            "C5GM2OB1".base32HexDecodeWord(0, 8),
