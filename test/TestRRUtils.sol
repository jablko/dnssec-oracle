...
contract TestRRUtils {
    ...
    function testLabelCount() public {
        ...
        Assert.equal(
            hex"016201610000".labelCount(0),
            2,
            "labelCount('b.a.') == 2"
        );
        Assert.equal(
            hex"066574686c61620378797a00".labelCount(6 + 1),
            1,
    ...
    function testIterateRRs() public {
        ...
        bytes memory rrs =
            hex"0161000001000100000e1000047400000101620161000001000100000e100004c0a80101";
    ...
    function testCheckTypeBitmapTextType() public {
        ...
        Assert.equal(
            tb.checkTypeBitmap(0, DNSTYPE_TEXT),
            true,
    ...
    function testCheckTypeBitmap() public {
        ...
        bytes memory tb =
            hex"FF0006400100000003041b000000000000000000000000000000000000000000000000000020";

        // Exists in bitmap
        Assert.equal(
            tb.checkTypeBitmap(1, DNSTYPE_A),
            true,
            "A record should exist in type bitmap"
        );
        // Does not exist, but in a window that is included
        Assert.equal(
            tb.checkTypeBitmap(1, DNSTYPE_CNAME),
            false,
            "CNAME record should not exist in type bitmap"
        );
        // Does not exist, past the end of a window that is included
        Assert.equal(
            tb.checkTypeBitmap(1, 64),
            false,
            "Type 64 should not exist in type bitmap"
        );
        // Does not exist, in a window that does not exist
        Assert.equal(
            tb.checkTypeBitmap(1, 769),
            false,
            "Type 769 should not exist in type bitmap"
        );
        // Exists in a subsequent window
        Assert.equal(
            tb.checkTypeBitmap(1, DNSTYPE_TYPE1234),
            true,
            "Type 1234 should exist in type bitmap"
        );
        // Does not exist, past the end of the bitmap windows
        Assert.equal(
            tb.checkTypeBitmap(1, 1281),
            false,
    ...
    function testcompareNames() public {
        ...
        Assert.equal(
            xyz.compareNames(ethLabXyz) < 0,
            true,
            "xyz comes before ethLab.xyz"
        );
        Assert.equal(
        ...
        Assert.equal(
            bthLabXyz.compareNames(xyz) > 0,
            true,
            "bthLab.xyz comes after xyz"
        );
    }
